...
# python -m pytest tests -m 'e2e' -v --disable-warnings ## To run only integration (e2e) tests (needs to start Flask app first with 'FLASK_APP=blog/app.py python -m flask run')
# python -m pytest tests -m 'not e2e'-v --disable-warnings ## To run all tests except integration
import json
import pathlib
import requests
import pytest
...
def validate_payload(payload, schema_name):
    ...
    #)
    schema = json.loads(pathlib.Path(f"./{schema_name}").read_text())   
    validate(
        payload,
        schema
    )
